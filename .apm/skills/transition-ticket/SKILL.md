...
2. Run `bash tickets.sh transition -t <code> -T <status>` using the extracted ticket code and canonical target status. The `transition` subcommand handles fuzzy matching and case-insensitive input for the target status.
