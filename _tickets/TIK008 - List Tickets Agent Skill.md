...
- The existing `.agents/skills/tickets/SKILL.md` and its parent directory must be deleted. This skill replaces it.

# Technical Solution

Delete the existing `.agents/skills/tickets/SKILL.md` and its directory, then create `.agents/skills/list-tickets/SKILL.md` following the standard skill format (frontmatter with `name: list-tickets` and a narrow `description` that limits its invocation scope to listing tickets only, then markdown instructions). The content maps the three natural-language keywords to the corresponding `tickets list --group` flags and provides a simple lookup table the agent can reference.

# Execution Plan

- [ ] Delete the existing `.agents/skills/tickets/SKILL.md` and its parent directory.
...
- [ ] Verify `tickets list --group active` lists existing tickets correctly.
